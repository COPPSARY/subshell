...
export function WorkspaceView({ project, tasks, onOpen, onOpenResult }: Props) {
  ...
  if (!project) return <div className="w-full p-7"><h1 className="text-base font-medium">Workspace dashboard</h1><p className="mt-2 text-sm text-secondary">Open a project to inspect agents, processes, worktrees, environments, and integration state.</p><GlobalSearch busy={busy} onOpen={open} onSearch={search} query={query} results={results} setQuery={setQuery} /></div>;
  ...
  return <div className="min-h-full w-full p-5 lg:p-7">
    ...
    <nav aria-label="Workspace sections" className="mt-4 flex gap-5 border-b border-line">
      {([['overview', 'Overview'], ['setup', 'Agent setup']] as const).map(([id, label]) => <button aria-current={view === id ? "page" : undefined} className="border-b-2 border-transparent px-0.5 py-2 text-xs text-secondary hover:text-primary aria-[current=page]:border-accent aria-[current=page]:text-primary" key={id} onClick={() => setView(id)} type="button">{label}</button>)}
    </nav>
    ...
    {view === "overview" ? <>
      ...
      <div className="mt-7 grid gap-6 md:grid-cols-2">
        ...
        <section className="md:col-span-2"><div className="flex items-center gap-2 border-b border-line pb-2"><BookmarkIcon size={14} /><h2 className="text-sm font-medium">Pinned items</h2><span className="ml-auto font-mono text-[10px] text-tertiary">{bookmarks.length}</span></div>{bookmarks.length ? <ul className="divide-y divide-line">{bookmarks.map((bookmark) => <li className="flex items-center gap-2 py-3 text-xs" key={bookmark.id}><button className="min-w-0 flex-1 truncate text-left" disabled={!bookmark.taskId} onClick={() => openBookmark(bookmark)} type="button">{bookmark.label || bookmark.runId || bookmark.taskId || bookmark.eventId}</button><button className="button-secondary" onClick={() => act(() => toggleBookmark(project.id, bookmark.eventId ? "event" : bookmark.runId ? "run" : "task", bookmark.eventId ?? bookmark.runId ?? bookmark.taskId!))} type="button"><Trash2 size={12} />Remove pin</button></li>)}</ul> : <p className="empty-row">Pin a search result to keep it here.</p>}</section>
      </div>
    </> : <>
      ...
      <div className="mt-3 max-w-4xl divide-y divide-line [&>section]:!border-l-0 [&>section]:!border-t-0 [&>section]:!px-0"><AgentTemplates busy={busy} onAct={act} projectId={project.id} providers={providers} templates={templates} /><EnvironmentProfiles busy={busy} onAct={act} profiles={profiles} projectId={project.id} /></div>
    </>}
  </div>;
}
