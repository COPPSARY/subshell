...
export function ReviewView({ taskId }: { taskId: string }) {
  ...
  useEffect(() => {
    let active = true;
    getReview(taskId).then((value) => active && setReview(value)).catch((reason) => active && setError(String(reason)));
    return () => { active = false; };
  }, [taskId]);

  async function act(action: () => Promise<Review>) {
    setBusy(true); setError("");
    try { setReview(await action()); } catch (reason) { setError(String(reason)); } finally { setBusy(false); }
  }

  async function merge() {
    if (!review) return;
    setBusy(true); setError("");
    try { setMergedRevision(await mergeReview(review.id, review.fingerprint)); } catch (reason) { setError(String(reason)); } finally { setBusy(false); }
  }
  ...
  return <div className="min-h-0 flex-1 overflow-auto bg-surface p-5">
    ...
    <section className="mt-5"><h3 className="text-sm font-medium">Exact combined diff</h3><pre className="mt-2 max-h-[32rem] overflow-auto rounded-md bg-app p-4 font-mono text-[11px] leading-5 text-secondary">{review.combinedPatch || "No file changes."}</pre></section>
