...
export function ProvidersView() {
  ...
  async function remove(id: string) { const profile = profiles.find((item) => item.id === id); if (!window.confirm(`Remove ${profile?.displayName ?? "this provider"} and its saved credential?`)) return; setError(""); try { await removeProvider(id); const removed = profiles.find((item) => item.id === id); setProfiles((items) => items.filter((item) => item.id !== id)); if (removed) setDetected((items) => items.map((item) => item.executablePath === removed.executablePath ? { ...item, isConfigured: false } : item)); } catch (reason) { setError(message(reason)); } }
