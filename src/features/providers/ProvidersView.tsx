...
import { createProvider, detectProviders, listProviders, removeProvider, updateProvider } from "./api";
import type { DetectedProvider, GenericProfile } from "./model";
import { ProviderIcon } from "./ProviderIcon";

const emptyProfile = (): GenericProfile => ({ id: "", displayName: "", executablePath: "", arguments: ["{prompt}"], resumeArguments: [], promptMode: "argument", configRootEnvVar: null, configSourcePath: null, inheritUserHome: false });
...
export function ProvidersView() {
  ...
  async function useDetected(provider: DetectedProvider) {
    setError("");
    try {
      const saved = await createProvider({ id: "", displayName: provider.displayName, executablePath: provider.executablePath, arguments: provider.arguments, resumeArguments: provider.resumeArguments, promptMode: provider.promptMode, configRootEnvVar: null, configSourcePath: null, inheritUserHome: true });
  ...
  async function remove(id: string) { await removeProvider(id); const removed = profiles.find((profile) => profile.id === id); setProfiles((items) => items.filter((item) => item.id !== id)); if (removed) setDetected((items) => items.map((item) => item.executablePath === removed.executablePath ? { ...item, isConfigured: false } : item)); }
  ...
  return <div className="w-full p-7">
    ...
    <p className="my-4 max-w-3xl text-sm leading-6 text-secondary">The first prompt automatically uses an installed coding agent and its existing CLI login. Use this screen only to choose another agent or add an unsupported CLI.</p>
