...
import { approveTaskPlan, completeRun, enqueueRuns, getTaskPlan, listRuns, previewRunEnvironment, rejectTaskPlan, resumeRun, startRuns, stopRun } from "./api";
...
export function RunWorkspace({ project, task, autoStart = false, initialRunId, onAutoStartConsumed, onActiveRunChange }: Props) {
  ...
  useEffect(() => { outputListeners.current.clear(); Promise.all([listProviders(), listContextSources(project.id), listRuns(task.id), getTaskPlan(task.id)]).then(([nextProviders, nextSources, existingRuns, taskPlan]) => { setProviders(nextProviders); setProvidersLoaded(true); setSources(nextSources); setRuns(existingRuns); setPlan(taskPlan); setActiveRunId(initialRunId && existingRuns.some((run) => run.id === initialRunId) ? initialRunId : existingRuns.find((run) => ["queued", "preparing", "running", "waiting"].includes(run.status))?.id ?? existingRuns.find((run) => run.role !== "planner" && run.status !== "succeeded")?.id ?? (reviewIsReady(existingRuns) ? "" : existingRuns[0]?.id ?? "")); setDrafts((items) => items.map((item) => ({ ...item, providerId: item.providerId || nextProviders[0]?.id || "" }))); }).catch((reason) => setError(errorMessage(reason))); }, [project.id, task.id]);
  ...
  const reviewReady = reviewIsReady(runs) || ["review", "approved"].includes(task.status);
