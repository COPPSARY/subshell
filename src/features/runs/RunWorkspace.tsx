...
type Draft = { providerId: string; instruction: string; role: string; selectedFiles: string[]; pattern: string; environmentFiles: string; environmentPreview: string[] | null; preview: ContextPreview | null; unitLimit: number | null };
const draft = (): Draft => ({ providerId: "", instruction: "", role: "implementer", selectedFiles: [], pattern: "", environmentFiles: "", environmentPreview: [], preview: null, unitLimit: null });
...
export function RunWorkspace({ project, task, autoStart = false, initialRunId, onAutoStartConsumed, onActiveRunChange }: Props) {
  ...
  useEffect(() => {
    ...
    Promise.resolve(providers[0] ?? detectProviders().then((installed) => {
      ...
      return createProvider({ id: "", displayName: detected.displayName, providerType: detected.key, executablePath: detected.executablePath, arguments: detected.arguments, resumeArguments: detected.resumeArguments, promptMode: detected.promptMode, configRootEnvVar: detected.configRootEnvVar, configSourcePath: null, inheritUserHome: true });
    }))
      .then(async (provider) => {
